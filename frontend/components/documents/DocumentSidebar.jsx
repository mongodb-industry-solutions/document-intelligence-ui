...
const DocumentSidebar = ({
  ...
  const handleDeleteConfirm = async () => {
    if (documentToDelete) {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/documents/${documentToDelete.document_id}`, {
          method: 'DELETE',
        });
